...
init python:
    ...
    def hnmgradientglow_tag(tag, argument, contents):
        return [
        (renpy.TEXT_TAG, u"shader=gradientglow:u__end_color=#ff0000:u__glow_color=#ffff00"),
    ] + contents + [
        (renpy.TEXT_TAG, u"/shader"),
    ]

    config.custom_text_tags["hnmgradient"] = hnmgradientglow_tag
    ## desired gradient for The Hananezumi Project text on the bottom-right of the main menu
    

    def chocola_colorsweep_tag(tag, argument, contents):
        return [
        (renpy.TEXT_TAG, u"shader=ColorSweep:u__color=#965b33:u__width=0.75:u__duration=1.5"),
    ] + contents + [
        (renpy.TEXT_TAG, u"/shader"),
    ]

    config.custom_text_tags["chocolacs"] = chocola_colorsweep_tag
    ## color sweep shader for Kuroshoko, not Chocola (Kuroshoko's identifier inside this code is Chocola, remember that)


    def satura_colorsweep_tag(tag, argument, contents):
        return [
        (renpy.TEXT_TAG, u"shader=ColorSweep:u__color=#ff5fef:u__width=0.75:u__duration=1.5"),
    ] + contents + [
        (renpy.TEXT_TAG, u"/shader"),
    ]

    config.custom_text_tags["saturacs"] = satura_colorsweep_tag
    ## color sweep shader for koibito Satura Asuki


    def maruu_colorsweep_tag(tag, argument, contents):
        return [
        (renpy.TEXT_TAG, u"shader=ColorSweep:u__color=#ffff00:u__width=0.75:u__duration=1.5"),
    ] + contents + [
        (renpy.TEXT_TAG, u"/shader"),
    ]

    config.custom_text_tags["maruucs"] = maruu_colorsweep_tag
    ## color sweep shader for HappyMaruu


    def splasht_colorsweep_tag(tag, argument, contents):
        return [
        (renpy.TEXT_TAG, u"shader=ColorSweep:u__color=#ffff00:u__width=0.25:u__duration=1.5"),
    ] + contents + [
        (renpy.TEXT_TAG, u"/shader"),
    ]

    config.custom_text_tags["splashtcs"] = splasht_colorsweep_tag
    ## color sweep shader for the confirmation text below of the splash




    # custom crap is off-limits below here
