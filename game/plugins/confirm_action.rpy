...
init python:
    ...
    class NotifyALT(Show):
        # Alternative to the classic CConfirm class above.

        def __init__(self, prompt, yes=None, no=None, confirm_selected=True,
                *args, **kwargs):

            if config.confirm_screen and renpy.has_screen('notifyalt'):
                screen = "notifyalt"
            elif renpy.has_screen("yesno_prompt"):
                screen = "yesno_prompt"
            else:
            ...
            super(NotifyALT, self).__init__(screen, config.exit_yesno_transition,
                *args, message=self.prompt, yes_action=self.yes, **kwargs)
        ...
        def __call__(self):
            ...
            if self.screen is None:
                return Confirm(self.prompt, self.yes, self.confirm_selected)()
            elif self.confirm_selected or not self.get_selected():
                return super(NotifyALT, self).__call__()
            else:
                return renpy.run(self.yes)
...
init python:
    ...
    class InfoBox(Show):
        ...
        def __init__(self, prompt, yes=None, no=None, confirm_selected=True,
            ...
            if config.confirm_screen and renpy.has_screen('infobox'):
                screen = "infobox"
            elif renpy.has_screen("yesno_prompt"):
                screen = "yesno_prompt"
            else:
            ...
            super(InfoBox, self).__init__(screen, config.exit_yesno_transition,
                *args, message=self.prompt, yes_action=self.yes, **kwargs)
        ...
        def __call__(self):
            ...
            if self.screen is None:
                ...
                return super(InfoBox, self).__call__()
            else:
                return renpy.run(self.yes)

init python:

    class InfoTimed(Show):
        # Alternative to the classic CConfirm class above.

        def __init__(self, prompt, confirm_selected=True,
                *args, **kwargs):

            if config.confirm_screen and renpy.has_screen('infotimed'):
                screen = "infotimed"
            else:
                screen = None
            ...
            super(InfoTimed, self).__init__(screen, config.exit_yesno_transition,
                *args, message=self.prompt, **kwargs)

        def __call__(self):

            if self.screen is None:
                return Confirm(self.prompt, self.confirm_selected)()
            elif self.confirm_selected or not self.get_selected():
                return super(InfoTimed, self).__call__()